...
def norm_raw(raw_list):
    if raw_list[2] == 1:
        # workday
        f1 = open('%sstat1.csv'%(dir_),encoding="utf-8")
        df = pd.read_csv(f1)
        df = df.reset_index()
        list1 = []
        ...
        # freeday
        f1 = open('%sstat2.csv'%(dir_),encoding="utf-8")
        df = pd.read_csv(f1)
        df = df.reset_index()
        list1 = []
        list1.append((raw_list[0]-df['x1_mean'][0])/(df['x1_div'][0]))
...
def pred(con):
    if con[2] == 1:
        ...
        net.load_state_dict(torch.load('%sregnet1.pt'%(dir_), map_location = 'cpu'))
        print('模型导入成功！')
        prediction = net(x)
        con.append(1)
        ...
        optimizer = torch.optim.SGD(net.parameters(), lr=0.01)  # 传入 net 的所有参数, 学习率
        loss_func = torch.nn.MSELoss()       # 预测值和真实值的误差计算公式 (均方差)
        net.load_state_dict(torch.load('%sregnet2.pt'%(dir_), map_location = 'cpu'))
        # print('模型导入成功！')
        prediction = net(x)
        con.append(0)
        prediction = norm_result(prediction.data.numpy()[0], con)
